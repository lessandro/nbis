...
#cat: txt2an2k - Parses a textually formatted version of an ANSI/NIST
#cat:            2007 file and writes its contents to new file in the
#cat:            standard compliant format.
...
void procargs(int argc, char **argv, char **ifile, char **ofile)
{
   ...
   if(argc != 3){
      fprintf(stderr, "Usage: %s <fmttext in> <ansi_nist out>\n", argv[0]);
      exit(-1);
   }
