...
# ******************************************************************************
#
# Makefile to loop through png library directories and call the next
# level Makefile.
#
# ******************************************************************************
libs:
	@( \
		for library in $(LIBRARYS); do \
			...
			(cd $(DIR_SRC_LIB)/$$library && $(MAKE) all) || exit 1; \
...
config:
	@( \
		for library in $(LIBRARYS); do \
			(cd $(DIR_SRC_LIB)/$$library && ./configure) || exit 1; \
		done;\
	);
#
install:
	@( \
		for library in $(LIBRARYS); do \
			(cd $(DIR_SRC_LIB)/$$library && $(MAKE) install_root) || exit 1; \
		done;\
	);
#
clean:
